...
b8 application_on_resized(u16 code, void* sender, void* listeber_inst, event_context context);
...
b8 application_on_resized(u16 code, void* sender, void* listeber_inst, event_context context)
{
    if(code == EVENT_CODE_RESIZED)
    {
        ...
        //Check if different. If true triggers a resize event.
        if(width != app_state.width || height != app_state.height)
        {
            app_state.width = width;
            app_state.height = height;

            DEBUG_LOG("Window resized: %i, %i", width, height);

            //Handle minimalization
            if(width == 0 || height == 0)
            {
