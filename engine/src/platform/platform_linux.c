...
b8 platform_startup(platform_state* plat_state, const char* application_name, i32 x, i32 y, i32 width, i32 height)
{
    ...
    {
        FATAL_LOG("FAILED to connect to X Server via XCB");
        return FALSE;
    }
    ...
    if(stream_result <= 0)
    {
        FATAL_LOG("An ERROR occured when flushing the stream: %d", stream_result);
        return FALSE;
    }
    return TRUE;
}
    ...
    while(event != 0)
    {
        ...
        switch(event -> response_type & ~0x80)
        {
            ...
            case XCB_KEY_RELEASE:
            {
                ...
                //Pass to the input subsys for further processing
            ...
            {
                /*Resizing - note that this is also triggered by moving the window, but should be passed anyway since a change in the x/y
                could mean an upper-left resize. The Application layer can decide what to do with this.*/
                xcb_configure_notify_event_t *configure_event = (xcb_configure_notify_event_t *)event;

                //Fire the event. The application layer should pick this up, but not handle it as it shouldn't be visible to other parts of the application.
