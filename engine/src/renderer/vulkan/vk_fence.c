...
b8 vulkan_fence_wait(vulkan_context* context, vulkan_fence* fence, u64 timeout_ns)
{
    if(!fence->is_signaled)
    {
        ...
        switch(result)
        {
            ...
            case VK_TIMEOUT:
            {
                WARN_LOG("vk_fence_wait <- Timed Out");
                break;
            }
            case VK_ERROR_DEVICE_LOST:
            {
                ERROR_LOG("vk_fence_wait <- VK_ERROR_DEVICE_LOST");
                break;
            }
            case VK_ERROR_OUT_OF_HOST_MEMORY:
            {
                ERROR_LOG("vk_fence_wait <- VK_ERROR_OUT_OF_HOST_MEMORY");
                break;
            }
            case VK_ERROR_OUT_OF_DEVICE_MEMORY:
            {
                ERROR_LOG("vk_fence_wait <- VK_ERROR_OUT_OF_DEVICE_MEMORY");
                break;
            }
            default:
            {
                ERROR_LOG("vk_fence_wait <- An unknown error has occurred");
                break;
            }
        }
    }
