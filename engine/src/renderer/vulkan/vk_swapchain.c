...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain)
{
    ...
    for(u32 i = 0; i < context->device.swapchain_support.format_count; ++i)
    {
        ...
        //Preferred formats(NOTE:If any error with it happens try: VK_COLOR_SPACE_SRGB_NONLINEAR_KHR)
        if(format.format == VK_FORMAT_B8G8R8A8_UNORM && format.colorSpace == VK_COLOR_SPACE_BT709_NONLINEAR_EXT)
        {
    ...
    swapchain_extent.width = CLAMP_VAL(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = CLAMP_VAL(swapchain_extent.height, min.height, max.height);
    ...
    if(!swapchain->images)
    {
        swapchain->images = (VkImage*)mem_allocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if(!swapchain->views)
    {
        swapchain->views = (VkImageView*)mem_allocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    ...
    if(!vulkan_device_detect_depth_format(&context->device))
    {
        ...
        FATAL_LOG("Failed to find a supported format!!!");
    }

    //Create depth image and its view
    vulkan_image_create(context, VK_IMAGE_TYPE_2D, swapchain_extent.width, swapchain_extent.height, context->device.depth_format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, true, VK_IMAGE_ASPECT_DEPTH_BIT, &swapchain->depth_attachment);
    INFO_LOG("Swapchain has been created successfully.")
}

void destroy(vulkan_context* context, vulkan_swapchain* swapchain)
{
    vulkan_image_destroy(context, &swapchain->depth_attachment);
    //Destroy the views. NOTE:It doesn't destroy images because they are owned by the swapchain and are destroued with it
    for(u32 i = 0; i < swapchain->image_count; ++i)
    {
