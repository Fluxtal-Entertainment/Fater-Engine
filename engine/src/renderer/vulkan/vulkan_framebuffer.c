...
{
    //Copies the atachements, renderpass and attachment count
    out_framebuffer->attachments = mem_allocate(sizeof(VkImageView) * attachment_count, MEMORY_TAG_RENDERER);
...
void vulkan_framebuffer_destroy(vulkan_context* context, vulkan_framebuffer* framebuffer)
{
    ...
    {
        mem_free(framebuffer->attachments, sizeof(VkImageView) * framebuffer->attachment_count, MEMORY_TAG_RENDERER);
