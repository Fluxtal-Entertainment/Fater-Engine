...
void vulkan_image_create(vulkan_context* context, VkImageType image_type, u32 width, u32 height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags memory_flags, b32 create_view, VkImageAspectFlags view_aspect_flags, vulkan_image* out_image)
{
    ...
    image_create_info.extent.depth = 1; //TODO:Support for configurable depth
    image_create_info.mipLevels = 4; //TODO:Support for mip mapping
    ...
    if(memory_type == -1)
    {
        ERROR_LOG("Required memory type not found! Image not valid.");
    }
...
void vulkan_image_view_create(vulkan_context* context, VkFormat format, vulkan_image* image, VkImageAspectFlags aspect_flags)
{
    ...
    view_create_info.viewType = VK_IMAGE_VIEW_TYPE_2D; //TODO:Make this configurable
